...
class BaseDatetime(SQLModel):
    created_at: Annotated[datetime, Field(default_factory=lambda: datetime.now(kst), sa_type=DateTime(timezone=True))]
    updated_at: Annotated[
        datetime,
        Field(
            default_factory=lambda: datetime.now(kst),
            sa_type=DateTime(timezone=True),
            sa_column_kwargs={"onupdate": lambda: datetime.now(kst)},  # sqlalchemy 문법사용
        ),
    ]
